...
class Site2 extends CI_Controller {
	...
	function alamat($id_produk = '', $kode = 0)
	{
		$this->countervisitor();
		$data['data_produk']=$this->model->GetDetailProduk("where tb_produk.id_produk='$kode'")->result_array();
		$data['data_keranjang']=$this->model->GetKeranjang()->result_array();
		// $data_content =  $this->blog_model->GetContentBlog("where content.kode_content = '$kode'")->result_array();
		$datas = array(
			"sidebar" => $this->sidebar_kat(),
	...
	function mkeranjang(){
		...
		$result = $this->model->Simpan('tb_keranjang', $data);
		if($result == 1){
			$this->session->set_flashdata("sukses", "<div class='alert alert-success'><strong>Simpan data BERHASIL dilakukan</strong></div>");
			header('location:'.base_url().'site2/alamat');
		}else{
	...
	function hapusker($kode){
		
		$result = $this->model->Hapus('tb_keranjang', array('id_keranjang' => $kode));
		if($result == 1){
